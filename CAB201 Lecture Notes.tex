...
In a UML class diagram, this is referred to as the generalisation of a derived class.
\subsection{Methods}
If a method is implemented in a base class, it can be accessed by a derived object, even if 
the derived class does not implement the method.

For example, B\@ has access to the \lstinline{PrintHello} method although it does not define it.
\begin{lstlisting}
class A
{
...
A virtual method provides a default implementation for a method which may be reimplemented
by derived classes. The method can be reimplemented using the \lstinline{override} keyword. 
If a derived class does not override a virtual method, the base classes implementation will be used.

For example, B reimplements the \lstinline{PrintHello} method, while C uses A's default implementation.
\begin{lstlisting}
class A
{
...
\end{lstlisting}
Note that the signature and modifiers must match the base class implementation.

Although B reimplements this method, it can still call the base classes implementation using the 
\lstinline{base} keyword.
\begin{lstlisting}
class A
{
    public virtual void PrintHello() { Console.WriteLine("Hello"); }
...
\end{lstlisting}
This technique should be utilised if a reimplementation only \textit{adds} to a base classes methods 
rather than modifying their implementations. This avoids repeating code.
...
the need to fully implement methods like in concrete classes. 

This allows us to declare abstract methods that have no implementation in 
the base class, but must be implemented (using \lstinline{override}) in all
derived classes. It follows that abstract classes cannot be instantiated.

To declare an abstract class or method, we can use the \lstinline{abstract} keyword.
Note that abstract methods can only be declared inside abstract classes.
\begin{lstlisting}
...
We can assign a \lstinline{Dog} and \lstinline{Cat} object to the base type \lstinline{Animal}, through inheritance.
Here \lstinline{Animal} is the static type, and \lstinline{Dog}/\lstinline{Cat} are dynamic types.
\subsection{Polymorphic Assignment}
Polymorphic assignment decides which method implementation is invoked given a virtual method
...
\end{lstlisting}
The program attempts to use the current classes implementation of \lstinline{Hello}, and if no override exists, the base class implementation is used.
\subsection{Interfaces}
...
Interfaces only contain method signatures which act as contracts so that all implementing classes must provide their own implementation.
...
Interfaces allow us to decouple code which creates better modular design by removing dependencies.
